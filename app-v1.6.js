...
const SYSTEMS = ['Cardiovascular','Endocrine/Metabolic','Fluid and Electrolytes','Gastrointestinal','Genitourinary/Renal','Immune/Hematology','Integument','Management of care','Musculoskeletal','Neurological','Oncology','Psychiatric','Reproductive','Respiratory','Sensory','Other'];
...
function normalizeRecord(record) {
  ...
  return {
    ...record,
    primarySystem: record.focusSystem || record.primarySystem || record.system || inferSystem(`${record.sessionTitle || ''} ${record.sessionTopic || ''} ${record.topic || ''}`),
    focusSystem: record.focusSystem || record.primarySystem || record.system || inferSystem(`${record.sessionTitle || ''} ${record.sessionTopic || ''} ${record.topic || ''}`),
    resource: record.resource || '',
    sessionTags: Array.isArray(record.sessionTags) ? record.sessionTags : [],
...
function inferSystem(value) {
  ...
  const matches = [
    ...
    ['Neurological', /neuro|brain|spinal|seizure/], ['Endocrine\/Metabolic', /endocr|diabet|thyroid|adrenal|metabolic/],
    ['Gastrointestinal', /gastro|digest|bowel|hepatic|liver/], ['Genitourinary\/Renal', /renal|urinary|kidney|genitourinary/],
    ['Musculoskeletal', /musculo|orthop|bone|joint/], ['Integument', /integument|skin|wound|burn/],
    ['Immune/Hematology', /hemat|blood|anemia|coag|immune|lymph|infection/],
    ['Reproductive', /reproduct|gyne|prostate|maternal|obstetric|pregnan|newborn/],
    ['Psychiatric', /mental|psychiatr/], ['Management of care', /management|delegat|priority/],
    ['Oncology', /oncolog|cancer|tumor/], ['Fluid and Electrolytes', /electrolyte|fluid balance|sodium|potassium/],
    ['Sensory', /sensory|vision|hearing|eye|ear/]
  ];
...
function render() {
  ...
  const filtered = state.records.filter((record) => {
    ...
    const matchesQuery = !query || `${record.question} ${record.primarySystem} ${record.sessionTitle} ${record.resource || ''} ${record.topic} ${(record.tags || []).join(' ')} ${record.notes} ${record.reasoning} ${record.rationale} ${record.remediationNotes}`.toLowerCase().includes(query);
        ...
        <button type="button" data-select-id="${record.id}" aria-label="Open ${escapeHtml(record.question)}">
          ...
          <div class="record-copy">
            ...
            <div class="record-meta">
              ${record.resource ? `<span>${escapeHtml(record.resource)}</span>` : ''}
...
function renderDetail() {
  ...
  $('#detailContent').innerHTML = `
    ...
    <span class="detail-topic">${escapeHtml(record.primarySystem)} · ${escapeHtml(record.sessionTitle)}${record.resource ? ` · ${escapeHtml(record.resource)}` : ''}</span>
...
function openQuestion(record = null) {
  ...
  $('#resourceInput').value = record?.resource || state.session?.resource || '';
...
function saveQuestion() {
  ...
  const record = {
    ...
    focusSystem: $('#systemInput').value,
    resource: $('#resourceInput').value.trim(),
    sessionTags: existing?.sessionTags || state.session?.tags || [],
...
function startSession() {
  if (!state.session) {
    ...
    const ordinalDay = day % 100 >= 11 && day % 100 <= 13 ? `${day}th` : `${day}${({1:'st',2:'nd',3:'rd'})[day % 10] || 'th'}`;
    const time = now.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true }).replace(/\s*[AP]M$/i, '');
    state.session = { id: uid(), startedAt: now.toISOString(), title: `${now.toLocaleDateString([], { weekday: 'long' })}, ${now.toLocaleDateString([], { month: 'short' })} ${ordinalDay} - ${time}` };
    saveState();
...
function importExtensionRecord(payload) {
  ...
  const record = {
    ...
    primarySystem: String(payload.focusSystem || payload.primarySystem || payload.system || existing?.primarySystem || 'Other').trim(),
    focusSystem: String(payload.focusSystem || payload.primarySystem || payload.system || existing?.focusSystem || existing?.primarySystem || 'Other').trim(),
    resource: String(payload.resource || existing?.resource || '').trim(),
    sessionTags: Array.isArray(payload.sessionTags) ? payload.sessionTags.map((tag) => String(tag).trim()).filter(Boolean) : (existing?.sessionTags || []),
